...
- 09:25 — AnkaThemeNeutralPickerOpen (Mondays only): H-2026-05-04-theme-detector-earnings-skip-v1 weekly NEUTRAL theme basket open. K=3 themes per Monday cutoff with `earnings_density==0` filter. Holdout 2026-05-04 → 2026-08-01 (auto-extend 2026-10-31 if n_NEUTRAL_cutoffs<6) (info)
...
- 14:30 — AnkaThemeNeutralPickerExitCheck: H-2026-05-04-theme-detector-earnings-skip-v1 mechanical T+21 TIME_STOP at Kite LTP for any basket whose target_close_date ≤ today (info)
...
**H-2026-05-04-theme-detector-earnings-skip-v1 (NEUTRAL-only weekly theme basket, K=3, density==0 filter):** PRE_REGISTERED 2026-05-02. Weekly Monday picker reads themes_proposer output, attaches V3-CURATED-30 PIT regime label at the cutoff, gates on regime==NEUTRAL, drops themes whose member stocks have any QUARTERLY_EARNINGS event in [cutoff, cutoff+21 trading days] (`earnings_density==0` filter), scores remainder with v1.4 raw-weight (7 factors), opens K=3 longs, holds 21 trading days, mechanical TIME_STOP at 14:30 IST. POOL forensic on NEUTRAL sub-panel: v1.4 top-K=3 mean fwd21 = +2.199% (vs unfiltered +1.554%); edge over random within filter = +0.645pp; bootstrap (1000 × 11-cutoff windows) edge median +0.465pp, **P(edge>0)=0.664** (gate ≥ 0.55). **Verdict bar (S1, deliberately ~50% below POOL point estimates to acknowledge post-hoc filter selection bias from 5 thresholds tested):** n_NEUTRAL_cutoffs ≥ 6, top-K mean fwd21 net@S1 ≥ +1.5%, edge_vs_random_within_filter ≥ +0.30pp, edge_vs_unfiltered_v14 ≥ +0.20pp, permutation p ≤ 0.10, fragility 2-blocks ≥ 2 positive. Single-touch holdout 2026-05-04 09:25 IST → 2026-08-01 14:30 IST, auto-extends until n_NEUTRAL_cutoffs ≥ 6 OR 2026-10-31. Predecessors: H-2026-05-01 (FAIL, full-POOL no skill), H-2026-05-03 (FAIL, regime-conditional Karpathy 528-recipe sweep TEST -1.13% vs v1.4 +1.76%; textbook overfit). Spec: `docs/superpowers/specs/2026-05-04-theme-detector-earnings-skip-v1-design.md`. Plan: `docs/superpowers/plans/2026-05-04-theme-detector-earnings-skip-v1.md`. Engine: `pipeline/research/h_2026_05_04_theme_neutral_skip/{picker,engine}.py`. Ledger: `pipeline/data/research/h_2026_05_04_theme_neutral_skip/recommendations.csv` (created on first OPEN). Lifecycle filter (novelty≥0.5 + phase∈{IGNITION,PRE_IGNITION,MATURE}) abandoned pre-registration after PRE_IGNITION mean fwd21 = -1.43% (anti-predictive); single-touch NOT consumed by lifecycle test. **Scheduled tasks (AnkaThemeNeutralPickerOpen Mondays 09:25 IST + AnkaThemeNeutralPickerExitCheck Mon-Fri 14:30 IST) on VPS systemd; units at `pipeline/infra/systemd/anka-theme-neutral-picker-{open,exit-check}.{service,timer}`.** **No parameter changes during the holdout window per backtesting-specs.txt §10.4 strict.**
...
**H-2026-05-04-cross-asset-perstock-lasso-v1 (per-stock cross-asset elastic-net, 101-stock F&O):** PRE_REGISTERED 2026-05-03; **A1 PRE_HOLDOUT_FIX 2026-05-03 13:52 IST + A2 GATE_RECONFIG 2026-05-03 17:00 IST.** Standards version 1.1_2026-05-03 (`backtesting-specs.txt §9C` added concurrently). Cell-level qualifier reduced from 6 gates to 2: GATE A mean fold-AUC ≥ 0.53 (was 0.55, aligns with López de Prado AFML §7.4 intraday equity classification median 0.52-0.54) AND GATE B BH-FDR p < 0.05 per-direction (LONG family ≈ 101, SHORT ≈ 101, was pooled n=202) at N_PERMUTATIONS=100,000 (was 10K, §9B.2 violation). Dropped per §9C.2: fold-AUC std (Pardo 2008 §6.7), in-sample-holdout AUC (leakage-prone), n_pred_pos absolute threshold (ESL §4.4.4: percentile ranking with shifted base rate), perm-beat-pct synthetic mapping (BH-FDR twice). Old-gate measurements retained as informational only. Single-touch preserved (pre-holdout, v1.0 produced 0 qualifiers, no positions opened). Diagnostic at `pipeline/research/h_2026_05_04_cross_asset_perstock_lasso/diagnostic_ta_only_results.json` (TA-only sanity baseline confirmed gates were mis-calibrated, not feature library failure — both feature sets produced flat fold-AUC distributions around 0.5). Re-fit overnight on VPS (~12hr at 100K perms). Per (stock, direction) cell: elastic-net logistic on 23 features (PCA K_ETF=10 reducing 30 CURATED foreign ETFs 1d returns + 4 Indian macro [Nifty near-month ×sqrt(1.5) emphasis + India VIX] + 6 stock TA + 3 DOW), exp-decay sample weights HL=90 trading days, 4-fold expanding-origin walk-forward, BH-FDR across cell grid. **Primary unit of inference: BASKET-level pass per spec §1.A — non-qualified cells are non-tradeable, NOT failed predictions.** Pre-flight 2026-05-03 (all PASS): K_ETF=10 at 85.4% var, max abs corr PC×TA = 0.079 (cross-asset truly orthogonal to TA), 101 stocks at ₹50cr ADV after sector-resolvable filter, ratio 5.66:1 at HL=90. **§1.B null bounds:** n_qualifying = 0=`FAIL_NO_QUALIFIERS` / [1,4]=`FAIL_INSUFFICIENT_QUALIFIERS` / [5,25]=expected / [26,80]=triggers §16.6 amplified leakage audit / >80=`FAIL_LEAKAGE_SUSPECT`. **§12 PASS bar:** n_qualifying ≥5, ≥60 trades, hit ≥55%, mean P&L ≥+0.4% net@S1, B0/B1/B3/B4 cleared, B2 negative. Single-touch holdout 2026-05-05 → 2026-08-05, auto-extend to 2026-10-31 if n_qualifying<5. **PRE_HOLDOUT_FIX amendment:** runner had broken sector_mapper import and a §5.3 coverage gap (only 11 of 24 sector_taxonomy keys map to a published Nifty sector index). Surfaced on first VPS deploy (0 cells fit). Fix: new `sector_mapping.py` module with explicit `SECTOR_TO_INDEX_FILE`, preflight tightened to require sector-resolvable, universe shrinks 200→101 (just clears the 100 §3 minimum). Holdout postponed 1 trading day. No scientific parameter changed. Spec: `docs/superpowers/specs/2026-05-04-cross-asset-perstock-lasso-v1-design.md`. Plan: `docs/superpowers/plans/2026-05-04-cross-asset-perstock-lasso-v1.md`. Pre-flight: `pipeline/research/h_2026_05_04_cross_asset_perstock_lasso/preflight_results.json`. Universe: `pipeline/research/h_2026_05_04_cross_asset_perstock_lasso/universe_frozen.json`. Engine: `pipeline/research/h_2026_05_04_cross_asset_perstock_lasso/{runner,predict_today,holdout_ledger,verdict,leakage_audit,sector_mapping}.py`. Ledger: `pipeline/research/h_2026_05_04_cross_asset_perstock_lasso/recommendations.csv` (created on first OPEN). **Scheduled tasks (AnkaCrossAssetPredict 04:30 + Open 09:15 + Close 14:25 + Recalibrate monthly) on VPS systemd; units at `pipeline/infra/systemd/anka-cross-asset-{predict,open,close,recalibrate}.{service,timer}`.** **No parameter changes during holdout per backtesting-specs.txt §10.4 strict.** 5d ETF horizon explicitly deferred to v2 (would push K_ETF to 18, violates Check 2 cap).
