...
def classify_event_claude(
    ...
    if not key:
        logger.warning("ANTHROPIC_API_KEY not set -- cannot use Tier 2 classification")
        return None, 0.0
...
def detect_new_events() -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        # Tier 2 fallback for low confidence
        if confidence < SIGNAL_CONFIDENCE_THRESHOLD and ANTHROPIC_API_KEY:
