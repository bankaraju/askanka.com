...
def test_forward_shape():
    n_etf, n_ctx, n_tickers = 120, 6, 211
...
def test_param_groups_have_right_weight_decay():
    model = EtfStockTailMlp(n_etf_features=120, n_context=6, n_tickers=211)
...
def test_seed_locked_reproducibility():
    ...
    m1 = EtfStockTailMlp(n_etf_features=120, n_context=6, n_tickers=211)
    torch.manual_seed(C.RANDOM_SEED)
    m2 = EtfStockTailMlp(n_etf_features=120, n_context=6, n_tickers=211)
