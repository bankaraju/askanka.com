...
The detector composes signals from 5 distinct data sources. Each is registered separately below. NULL entries for Phase 2 sources are placeholders pending data acquisition.

| ID | Source | Phase | Status |
|---|---|---|---|
| TD-D1 | NIFTY-500 / NIFTY-50 free-float weight history | 1 | DATA ACQUISITION REQUIRED |
| TD-D2 | Sectoral indices daily history | 1 | EXISTING — Kite + fno_historical |
| TD-D3 | NSE main-board IPO calendar | 1 | DATA ACQUISITION REQUIRED |
| TD-D4 | M&A corporate announcements | 2 | DATA ACQUISITION REQUIRED |
| TD-D5 | Capex disclosure announcements | 2 | NLP LAYER REQUIRED |
| TD-D6 (auxiliary) | NIFTY-500 constituent membership history | 1 | DATA ACQUISITION REQUIRED (for retro-backfill correctness) |
...
- TD-D1, TD-D2, TD-D3, TD-D6 are operational and pass cleanliness gates
- TD-D4, TD-D5 are operational OR the detector emits `null` scores for those families with explicit `data_unavailable` flag
- Retro-backfill validation per design doc §8 passes
- The detector itself is registered (this audit doc + design doc both committed)
- A first weekly run completes successfully
...
- `pipeline/config/anka_inventory.json` — add data-collection task entries for TD-D1, TD-D3, TD-D6
- `docs/SYSTEM_OPERATIONS_MANUAL.md` — document data sources
