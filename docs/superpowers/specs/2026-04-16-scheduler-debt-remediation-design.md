...
| 63 | Phase A not scheduled | No task runs `reverse_regime_analysis.py`; `reverse_regime_profile.json` = Apr 14 (2-day stale) | (iii) interim (i) | — | Phase B runs with stale profile — live data-integrity issue | Schedule AnkaReverseRegimeProfile @ 04:45 IST (Batch C2) as INTERIM | DONE-INTERIM (task registered; scheduler-driven run LastResult=0 @ 2026-04-16 12:38:08; NextRun 2026-04-17 04:45; profile refreshed to 3.5MB at pipeline/autoresearch/) |
| 64 | AnkaCorrelationBreaks CLI | `correlation_breaks.bat` passes `--day 1 --no-telegram` which argparse rejects | (i) | 2026-04-14-correlation-break-detector §CLI | `.bat` written against older CLI | Strip those args from .bat (Batch C1) | DONE (`7e99c1c` — manual run exit 0; correlation_breaks.json + correlation_break_history.json written today 12:36) |
| 65 | Apr 14 15:38 cluster | 18 `pipeline/data/*.json` files share exact mtime Apr 14 15:38 (snapshotted in A3) | (i) master-job | 2026-04-14-unified-repo-clockwork (task registry) | Hypothesis wrong: not one master, **8 distinct writers** (5 untracked) — see C3 transcript writer map | **DEFERRED-NEW** — script rot + missing orchestrator, not scheduler debt. C3 investigation: `correlation_regime.py` BROKEN (`ImportError CORRELATION_PAIRS`); `macro_stress.py`/`model_drift.py` exit 0 but silently no-op; `pattern_engine.py` partial (pattern_lookup refreshed, historical_events didn't); 5 writers untracked (gamma_scanner, options_monitor, pinning_detector, unified_regime_engine, expiry_monitor). Needs separate mini-plan with per-writer triage + orchestrator design. Snapshots in `pipeline/backups/data_snapshots/2026-04-16/` are the baseline. Partial win: `pattern_lookup.json` refreshed today. | DEFERRED-NEW |
| 66 | gamma_result.json orphan | 231 bytes; no committed writer in codebase | (ii) hygiene | — | Delete or resolve via gamma_scanner.py untracked script | Resolve in Batch C3.x | DONE — orphan confirmed (Gamma.app presentation tool artifact, unrelated to pipeline GEX); deleted. `gamma_scanner.py` committed separately (purposeful, dark-for-now — triage under DEFERRED-NEW mini-plan per row 65) |
| 67 | options_monitor.py untracked | `git status` shows untracked; writes oi_history.json | (ii) hygiene | — | Single-repo mandate violation | Commit or delete in Batch C3.x | DONE — committed; 3 tracked consumers (`unified_regime_engine.py:179`, `regime_signals.py:222`, `regime_playbook.py:254`) all import `fetch_nifty_oi` so leaving it untracked would ImportError on any clone/CI |
